...
# ==============================================================================
# TRsync - Q4OS Installer (.qsi) Builder
# ==============================================================================
...
echo "=================================================="
echo " TRsync - Q4OS .qsi Package Builder"
...
find_latest_deb() {
    local debs=("$SCRIPT_DIR"/trsync_[0-9]*_*.deb "$SCRIPT_DIR"/*.deb "$SCRIPT_DIR"/build/*.deb)
    for f in "${debs[@]}"; do
        if [ -f "$f" ]; then
            echo "$f"
            return 0
...
PACKAGE_NAME=$(dpkg-deb -f "$LATEST_DEB" Package 2>/dev/null || echo "trsync")
DEB_VERSION=$(dpkg-deb -f "$LATEST_DEB" Version 2>/dev/null || echo "$APP_VERSION")
...
APPNAME_DESC="TRsync File Synchronization"
APP_ICON="trsync"
PK_VERS="$DEB_VERSION"
SETUP_TYPE="2"
